...
export default {
  ...
  methods: {
    async getPayInfo() {
      const result = await this.$API.requPayInfo(this.orderId);
      if (result.code == 200) {
    ...
    async open() {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          //发送请求 获取用户支付状态
          let result = await this.$API.requPayStatus(this.orderId);
          console.log(result);
          if (result.code == 200) {
            //清空定时器
            clearInterval(this.timer);
